...
const SRC = path.join(ROOT, 'src', 'ephemeris.js');
...
function readSource() {
  ...
  if (!marks.length) throw new Error('no section banners found in src/ephemeris.js');
...
function assertModesAreIndependent(modes) {
  ...
  if (problems.length) {
    throw new Error('modes must depend only on the core, but:\n' + problems.join('\n') +
      '\nMove the shared helper into the core section at the top of src/ephemeris.js.');
  }
}
...
(async () => {
  ...
  const licence = `/*! ephemeris ${version} | MIT | github.com/TonkaTuff/ephemeris */\n`;

  // the body table, read straight out of the source so this never drifts
  const Ephemeris = require(SRC);
  const bodies = {};
  for (const [name, b] of Object.entries(Ephemeris.BODIES)) bodies[name] = b;
  const canonical = new Set(Object.values(Ephemeris.GROUPS).flat());
  ...
  console.log(`ephemeris ${version} -> dist/  (${rows.length} files)\n`);
